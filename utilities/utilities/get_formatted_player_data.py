...
def main():
    ...
    if args.two_teams_fix:
        new_rows, errors = format_two_team_rows(player_data)
        save_new_rows(player_data=player_data, filename=args.output, new_rows=new_rows, errors=errors)

        if args.input:
            player_data = pd.read_csv(player_data_path, names=HEADERS)
            
        player_data = process_new_rows(player_data=player_data, new_rows=new_rows)
        
        if args.validate_output:
            validator(args.output, player_data)
...
def validator(output: str, player_data: pd.DataFrame):
    print("Validating output files...")
    original_path = os.path.join(CURRENT_DIR, '_temp.'.join(output.split('.')))
    ...
    print(message)
    message_path = os.path.join(CURRENT_DIR, output.split('.')[0] + '_validation.txt')
    ...
    errors_path = os.path.join(CURRENT_DIR, output.split('.')[0] + '_errors.json')
    with open(errors_path, 'w') as errors_json:
        json.dump(missing, errors_json)
    print(f'Validation output saved to {errors_path}')


def save_new_rows(player_data: pd.DataFrame, filename: str, new_rows: list, errors: list):
    ...
    if errors:
        errors_path = '_new_rows_errors.'.join(filename.split('.'))
        with open(os.path.join(CURRENT_DIR, errors_path), 'w') as f:
            wr = csv.writer(f, quoting=csv.QUOTE_ALL)
            wr.writerows(errors)


def process_new_rows(player_data: pd.DataFrame,new_rows: list) -> pd.DataFrame:
